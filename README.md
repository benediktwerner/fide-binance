...
2. *Optional:* Create a venv and activate it: `python3 -m venv venv && . ./venv/bin/activate` 
...
5. Edit `PLAYER_MAP` in `pairing.py`:
   - Mapping `name` -> `username`, then use `name` to refer to that player, usernames won't work
...
For more complex commands, the repo contains empty files with their names which should allow your shell to autocomplete them.
...
- Ensure valid tokens are present for all players in a round: `pairing.py ensure_tokens 42`
  ...
  - This requires a `web:mod` token for an account with the Challenge Admin permission in the `.env` file.
...
- Start clocks of a single game: `pairing.py start_clock abcdefgh` (`abcdefgh` is the Lichess ID of the game)
